...
class ComprehensiveSeeder {
  constructor() {
    this.departments = [
      ...
      },
      {
        ...
        code: "REVENUE",
        description:
          "Manages taxation, revenue collection, and financial administration",
        categories: ["other"],
        contactInfo: {
        ...
        for (let i = 0; i < grievancesToCreate; i++) {
          ...
          if (status !== "pending") {
            const departmentOfficers = officers.filter(
              (o) =>
                o.department ===
                this.getDepartmentByCategory(category).toUpperCase()
            );
  ...
  generateStatusUpdates(finalStatus, createdDate, assignedOfficer) {
    ...
    const updateMessages = {
      ...
      rejected:
        "Case has been reviewed and rejected due to insufficient information or invalid complaint",
    };
...
const seedComprehensiveData = async () => {
  try {
    ...
    categoryStats.forEach((stat) => {
      console.log(
        `   • ${stat._id}: ${stat.count} grievances (avg urgency: ${Math.round(
          stat.avgUrgency
        )})`
      );
    });
